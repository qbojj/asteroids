...
{ 
	...
	if( pView->m_bSimulateGame && !pView->m_oGameThread.m_bInside ) 
	{
...
CAsteroidsView::CAsteroidsView()
{
	...
	m_uiResolution = max( tc.wPeriodMin, min( tc.wPeriodMax, WANTED_RES ) );
...
void CAsteroidsView::GenerateEnemies( int TimeDiff )
{
	...
	double Seconds = TimeDiff * 0.0001;//( m_iTicks * CLOCK_DELAY ) * 0.001;

	m_dLevel = min( 7000 + Seconds * 1.5 + Seconds * Seconds * 0.1, 100000 );
...
void CAsteroidsView::GameTick()
{
	...
	if( m_bEnemyGen ) GenerateEnemies( TimeDiff );
...
void CAsteroidsView::OnTimer(UINT nIDEvent) 
{
	...
	{
		if( m_bSimulateGame && !m_oGameThread.m_bThreadExists )
		{
			//AfxMessageBox( "Lost thread", 0, 0 );
			StartGameThread();
		}
	}
...
void CAsteroidsView::OnInitialUpdate() 
{
	...
	if( !m_bInitialized )
	{
		((CAsteroidsApp *)AfxGetApp())->m_hViewHWND = m_hWnd;
...
int CAsteroidsView::DrawGame( int Bitmap /*= -1*/ )
{
	...
	CGdiObject *LastPen = m_oGameDC[ Bitmap ].SelectStockObject( WHITE_PEN );

	m_poGameObjs->Draw( &m_oGameDC[ Bitmap ], m_bShowScore, m_dConv );
