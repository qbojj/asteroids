...
CGameThread::CGameThread() : m_oAnn( ANN_LOCATION )
{
	m_bEnemyGen = false;
	m_bShowScore = false;
	m_dConv = 1;
...
UINT CGameThread::ThreadFunction( LPVOID pParam )
{
	...
	while( 1 )
	{
		...
		if( Result == WAIT_OBJECT_0 )
		{
			...
			float In[128];
			CNNDataCollector::GetAnnInput(pGameThreadClass->m_pGameObjsArray, In);
			const float *pControlls = kann_apply1(pGameThreadClass->m_oAnn.m_pKann, In);
			for (int i = 0; i < 5; i++) 
				pGameThreadClass->m_pGameObjsArray->m_oShip.m_aControls[i] = pControlls[i] >= .5 ? 1 : 0;
