...
#if FPS_DISP
	int i;
	for( i = 0; i < TIME_MEASURE_CELLS; i++ ) m_iElapse[ i ] = 0;
	m_iPos = 0;
	m_iSum = 0;
...
#if RECORD
			m_oGameThread.m_oDataCollector.Save( m_poGameObjs->m_iScore );
#endif
...
void CAsteroidsView::InitGame()
{
	...
	m_oGameThread.m_bLogGame = 1;
	m_oGameThread.m_iTickNum = 0;

	m_oGameThread.m_uiGameLen = 0;

#if !RECORD
	m_oGameThread.m_oDataCollector.Load( REPLAY_FILE );
#endif
