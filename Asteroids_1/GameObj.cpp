...
void CGameObj::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	pDC->Ellipse( round( Center.x - Radius ), round( Center.y - Radius ), round( Center.x + Radius ), round( Center.y + Radius ) );
}
	...
	if( ! m_bForDestruction && m_iInvisible <= 0 && m_iKind != KIND_CORPSE )
	{
		...
		while( CurrentObj )
		{
			if( ! CurrentObj->m_bForDestruction && CurrentObj->m_iTeam != m_iTeam && CurrentObj->m_iInvisible <= 0 && CurrentObj->m_iKind != KIND_CORPSE )
			{
				...
				double udDistance = sqrt( dX * dX + dY * dY );
				double udMinDistance = m_dHitboxRadius + CurrentObj->m_dHitboxRadius;
				
				bCollision = udDistance <= udMinDistance;
...
void CShip::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	if( ! m_bForDestruction )
	{
		CGdiObject *pLastObj = pDC->SelectStockObject( NULL_BRUSH );

		if( m_iShield ) pDC->Ellipse( round( Center.x - ( m_dHitboxRadius + 3 ) * Conv ), round( Center.y - ( m_dHitboxRadius + 3 ) * Conv ), round( Center.x + ( m_dHitboxRadius + 3 ) * Conv ), round( Center.y + ( m_dHitboxRadius + 3 ) * Conv ) );
		...
		pDC->Polygon( (CPoint *)&Points, 3 );

		if( m_aControls[ ACC ] )
		{
			CCoord FireTip( cos( m_dDirection + PI ) * Radius * 1.7, sin( m_dDirection + PI ) * Radius * 1.7 );

			pDC->MoveTo( Center + GetPointAtPercent( LWing, RWing, 0.3 ) );
			pDC->LineTo( Center + FireTip );
			pDC->LineTo( Center + GetPointAtPercent( LWing, RWing, 0.7 ) );
		}

		pDC->SelectObject( pLastObj );
	}
}
		...
		for(; A >= 0 && i < NUM_OF_RINGS; A -= RING_SPACING, i++ )
		{
			if( A <= 1 )
			{
				if( A < HYP_SPC_DIVISION )
				{
					...
					pDC->Ellipse( CRect( GetPointAtPercent( m_oCenter * Conv, TL, GrowthPercent ), GetPointAtPercent( m_oCenter * Conv, BR, GrowthPercent ) ) );
				}
				else
				{
					double SlidePercent = ( A - HYP_SPC_DIVISION ) / ( 1 - HYP_SPC_DIVISION );

					pDC->Ellipse( CRect( GetPointAtPercent( TL, m_oHypSpcNewPos * Conv, SlidePercent ), GetPointAtPercent( BR, m_oHypSpcNewPos * Conv, SlidePercent ) ) );
				}
			}
		}
	}
...
void CShip::Tick( int TimeDiff )
{
	if( ! m_bForDestruction && TimeDiff > 0 )
	{
		...
		{
			m_dDirection += 0.003 * ( (int)m_aControls[ RIGHT - SHIP_KEY_BIND_START ] - (int)m_aControls[ LEFT - SHIP_KEY_BIND_START ] );

			if( m_aControls[ ACC ] ) Accelerate();
			Drag();

			if( m_iReload > 0 )	m_iReload--;
			else if( m_aControls[ SHOOT ] )	Shoot( TimeDiff - i - 1 );
		
			m_oCenter += m_oMovementVector;
		}
...
{
	m_oMovementVector.x += cos( m_dDirection ) * 0.0003;
	m_oMovementVector.y += sin( m_dDirection ) * 0.0003;
}

void CShip::Drag()
{
	m_oMovementVector.x -= fabs( m_oMovementVector.x ) * m_oMovementVector.x * DRAG;
	m_oMovementVector.y -= fabs( m_oMovementVector.y ) * m_oMovementVector.y * DRAG;
}
...
void CAsteroid::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	pDC->MoveTo( Asteroid[ 10 ] );

	for( i = 0; i < 11; i++ )
	{	
		pDC->LineTo( Asteroid[ i ] );
	}
}
...
void CUfo::Accelerate()
{
	m_oMovementVector.x += cos( m_dPlayerDirection ) * 0.00007;
	m_oMovementVector.y += sin( m_dPlayerDirection ) * 0.00007;
}
...
void CCorpse::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	for( int i = 0; i < 10; i++ )
		pDC->SetPixel( GetPointAtPercent( m_oCenter, m_oCenter + m_aParticleDest[ i ], Percent ) * Conv, RGB( 255, 255, 255 ) );
}
...
void CShipCorpse::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	for( int i = 0; i < 5; i++ )
	{
		if( m_iLifespan >= m_aShowTimes[ i ] )
		{
			...
			pDC->LineTo( StartPoint + m_aParticleDest[ i * 2 + 1 ] * Conv );
		}
	}
}