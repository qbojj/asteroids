...
void CNNDataCollector::Save( int Score )
{
	...
	sprintf( Name, ".\\dat\\Replays\\%d_%s.rep", Score, Time.Format( "%d%b%Y, %H %M'%S''" ).GetBuffer() );
...
// ||a||*||b||*cos( <(a,b) )
static inline double dot( const CCoord &a, const CCoord &b ) { return a.x*b.x + a.y*b.y; }
static inline double det( const CCoord &a, const CCoord &b ) { return a.x*b.y - b.x*a.y; };

static inline void getLine(const CCoord &a, const CCoord &b, double &_a, double &_b, double &_c)
{
...
// distance from line through l1 & l2 to point p
static double dist(const CCoord &l1, const CCoord &l2, const CCoord &p)
{
    double a, b, c;
    getLine(l1, l2, a, b, c);
...
void CNNDataCollector::GetAnnInput( CGameObjsArray *pObjsArray, float *Input )
{
	// Ship info : 10
	Input[ 0 ] = (float) pObjsArray->m_oShip.m_iShield;
	Input[ 1 ] = (float) pObjsArray->m_oShip.m_iHypSpaceCooldown;
	...
	const int NumOfRays = 256 - 10;

	static CCoord Lines[NumOfRays];
	const CCoord Zero(0,0);

	CCoord Rot( 1, 0 );
	Rot.Rotate( Zero, -pObjsArray->m_oShip.m_dDirection );
	...
	if( !Initialized )
	{
		for( int i = 0; i < NumOfRays; i++ )
		{
			...
			Lines[i].x = 1, Lines[i].y = 0;
			Lines[i].Rotate( Zero, 2*PI / NumOfRays * i );
		}
	...
	while( pos )
	{
		...
		// Dif is now posision of pObj as ship was in (0,0)

		//Dif.Rotate( Center, -pObjsArray->m_oShip.m_dDirection );
		// complex mult (rotate -m_oShip.m_dDirection )
		CCoord RelativePos( Dif.x * Rot.x - Dif.y * Rot.y, Dif.x * Rot.y + Dif.y * Rot.x );

		for( int i = 0; i < NumOfRays; i++ )
		{
			// check if object is on correct side
			if( dot( Lines[i], Dif ) < 0 ) continue;

			double ObjDist = dist( Zero, Lines[i], RelativePos );
			...
			Input[ 10 + i ] = (float)min( Input[ 10 + i ], TrueDist );
		}
	}
}
