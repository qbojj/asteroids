...
static kad_rng_t kad_rng_dat = { {0x50f5647d2380309dui64, 0x91ffa96fc4c62cceui64 }, 0.0, 0, 0 };

static inline uint64_t kad_splitmix64(uint64_t x)
{
	uint64_t z = (x += 0x9E3779B97F4A7C15ui64);
...
{
	static const uint64_t JUMP[] = { 0xbeac0467eba5facbui64, 0xd86b048b86aa9922ui64 };
	uint64_t s0 = 0, s1 = 0;
	int i, b;
	for (i = 0; i < 2; ++i)
		for (b = 0; b < 64; b++) {
...
double kad_drand(void *d)
{
	...
	u.i = 0x3FFui64 << 52 | kad_xoroshiro128plus_next(d? (kad_rng_t*)d : &kad_rng_dat) >> 12;
	return u.d - 1.0;
}
...
int kad_op_stdnorm(kad_node_t *p, int action)
{
	...
	if (action == KAD_SYNC_DIM) {
		...
		for (j = 0; j < m; ++j) {
			...
			s /= n, t /= n;
			for (i = 0; i < n; ++i)
				qg[i] += (float) (std_inv * (pg[i] - s - px[i] * t));
		}
	}
	return 0;
}
...
static void conv2d_move_1to3(long d[4], const float *x, float *y) /* convert the NCHW shape to the NHWC shape */
{
	int i, j, k, l;
	for (i = 0; i < d[0]; ++i)
		for (j = 0; j < d[1]; ++j)
...
static void conv2d_add_3to1(long d[4], const float *y, float *x) /* convert the NHWC shape back to NCHW and add to another NCHW-shaped array */
{
	int i, j, k, l;
	for (i = 0; i < d[0]; ++i)
		for (j = 0; j < d[1]; ++j)
...
static void conv1d_move_1to2(long d[3], const float *x, float *y)
{
	int i, j, k;
	for (k = 0; k < d[0]; ++k)
		for (j = 0; j < d[1]; ++j)
...
static void conv1d_add_2to1(long d[3], const float *y, float *x)
{
	int i, j, k;
	for (k = 0; k < d[0]; ++k)
		for (j = 0; j < d[1]; ++j)
