...
CGameObjsArray::CGameObjsArray()
{
	...
	m_sndDestroy.CreateFromFile( _T("snd\\explode.wav") );
	m_sndUFODestroy.CreateFromFile( _T( "snd\\ufoend.wav") );
		...
		if( ! m_aObjects.IsEmpty() )
		{
			...
			do
			{
				...
				if( obj->m_bForDestruction )
				{
					...
					if( obj->m_iKind == KIND_UFO_NEUTRAL || obj->m_iKind == KIND_UFO_CHASING ) 
					{
...
void CGameObjsArray::Draw( CDC *pDC, bool ShowScore /*= true*/, double Conv /*= 1*/ )
{
	...
	if( ShowScore )
	{
		...
		double HitboxRadius = SHIP_SIZE * GAME_OBJ_SIZE_MUL * 0.75;
		double Direction = PI * 1.5;

		for( int i = 1; i <= m_oShip.m_iLifes; i++ ) 
			CShip::DrawShip( pDC, CCoord( HitboxRadius * i , HitboxRadius ) * 2 * Conv, Conv, 
							 Direction, HitboxRadius, 0, 0 );
	}
}
...
void CGameObjsArray::AddObj( CGameObj *pNewObj )
{
	if( pNewObj )
	{
		...
		if( pNewObj->m_iKind == KIND_ASTEROID || 
			pNewObj->m_iKind == KIND_UFO_NEUTRAL || 
			pNewObj->m_iKind == KIND_UFO_CHASING ) m_iEnemies++;
	}
}

void CGameObjsArray::DeleteAll()
{
	while( !m_aObjects.IsEmpty() ) free( m_aObjects.RemoveHead() );
