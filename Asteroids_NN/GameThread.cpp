...
UINT CGameThread::ThreadFunction( LPVOID pParam )
{
	...
	while( 1 )
	{
		...
		if( Result == WAIT_OBJECT_0 )
		{
			...
			if( pGameThreadClass->m_bLogGame )
			{
				pGameThreadClass->m_uiTimeFromLastLog += pGameThreadClass->m_uiTimeDiff;

				if( pGameThreadClass->m_uiTimeFromLastLog > 16 )
				{
					pGameThreadClass->m_uiTimeFromLastLog -= 16;
#if RECORD
					pGameThreadClass->m_oDataCollector.LogMoment( pGameThreadClass->m_pGameObjsArray );
#else
					pGameThreadClass->m_oDataCollector.ImportMoment( 
						pGameThreadClass->m_iTickNum++, pGameThreadClass->m_pGameObjsArray );
#endif
				}
			}
		...
		{
#if RECORD
			pGameThreadClass->m_oDataCollector.Save( pGameThreadClass->m_pGameObjsArray->m_iScore );
#else
			pGameThreadClass->m_oDataCollector.Clear();
#endif
			break;
		}
	}
...
{
	double Level = GetDifficulty( m_uiGameLen * 0.001 );
...
#endif
			pNewObj = new CAsteroid( NewObjPos, NewObjSpeed );
