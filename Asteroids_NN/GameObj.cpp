...
{
	if( ! m_bForDestruction && m_iInvisible <= 0 && ( m_iKind != KIND_CORPSE && m_iKind != KIND_CORPSE_SHIP ) )
	{
		...
		while( CurrentObj )
		{
			if( ! CurrentObj->m_bForDestruction && CurrentObj->m_iTeam != m_iTeam && CurrentObj->m_iInvisible <= 0 && ( CurrentObj->m_iKind != KIND_CORPSE && CurrentObj->m_iKind != KIND_CORPSE_SHIP ) )
			{
...
CDirectSound CShip::m_sndLaser;
CDirectSound CShip::m_sndBigLaser;
CDirectSound CShip::m_sndHyperspace;

CShip::CShip() : CGameObj( CCoord( GAME_WIDTH * 0.5, GAME_HEIGHT * 0.5 ), CCoord( 0, 0 ) )
{
	m_dDirection = 0;
	m_dHitboxRadius = SHIP_SIZE * GAME_OBJ_SIZE_MUL;
	...
	m_iSizeOf = sizeof( CShip );

	if( !m_sndLaser.IsValid() )
	{
		m_sndLaser.CreateFromFile( _T("snd\\lasersmall.wav") );
		m_sndLaser.SetVolume( -12 );
	}

	if( !m_sndBigLaser.IsValid() )
	{
		m_sndBigLaser.CreateFromFile( _T( "snd\\laserbig.wav") );
		m_sndBigLaser.SetVolume( -12 );
	}

	if( !m_sndHyperspace.IsValid() ) m_sndHyperspace.CreateFromFile( _T( "snd\\hyperspace.wav" ) );
...
void CShip::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	if( ! m_bForDestruction )
	{
		DrawShip( pDC, Center, Conv, m_dDirection, m_dHitboxRadius, m_iShield > 0, m_aControls[ ACC ] );
	}
}
...
void CShip::SetBulletNum( int Bullets )
{
	m_iBullets = Bullets;

	m_iShootReloadTime = (int)( 128 * ( 1 + ( Bullets - 1 ) * 0.5 ) );
}

void CShip::DrawShip( CDC *pDC, CCoord Center, double Conv, double direction, double size, bool shield, bool thrust )
{
	if( shield )
		DrawEllipse( pDC, Round( Center.x - ( size + 3 ) * Conv ), 
						  Round( Center.y - ( size + 3 ) * Conv ), 
						  Round( Center.x + ( size + 3 ) * Conv ), 
						  Round( Center.y + ( size + 3 ) * Conv ), GAME_COLOR );

	double Radius = size * Conv;
	double dHeadToArmAngle = PI * 15 / 18;

	CCoord head( cos( direction ) * Radius, sin( direction ) * Radius );

	CCoord RWing( cos( direction - dHeadToArmAngle ) * Radius, 
				  sin( direction - dHeadToArmAngle ) * Radius );

	CCoord LWing( cos( direction + dHeadToArmAngle ) * Radius,
				  sin( direction + dHeadToArmAngle ) * Radius );
	...
	if( thrust )
	{
		CCoord FireTip( cos( direction + PI ) * Radius * 1.7, sin( direction + PI ) * Radius * 1.7 );
...
CUfo::CUfo( const CCoord &StartPos, const CCoord &StartVector, int type ) : CGameObj( StartPos, StartVector )
{
	...
	if( ! m_sndUFO.IsValid() ) m_sndUFO.CreateFromFile( _T("snd\\ufo.wav") );

	m_sndUFO.Play();

	m_iKind = type == 0 ? KIND_UFO_NEUTRAL : KIND_UFO_CHASING;

	m_iSizeOf = sizeof( CUfo );
}
...
void CCorpse::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	for( int i = 0; i < 10; i++ )
		pDC->SetPixelV( GetPointAtPercent( m_oCenter, m_oCenter + m_aParticleDest[ i ], Percent ) * Conv, GAME_COLOR );
}
...
CShipCorpse::CShipCorpse( const CCoord &StartPos, const CCoord &StartVector ) : CCorpse( StartPos, StartVector )
{
	...
	m_iKind = KIND_CORPSE_SHIP;

	m_iSizeOf = sizeof( CShipCorpse );
}
