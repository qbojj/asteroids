...
void DrawLine(CDC *pDC, int x1, int y1, int x2, int y2, COLORREF color)
{
	...
	if (abs(dx) > abs(dy))
	{
		...
		for (xs=x1+1; xs<x2; xs++)
		{
			...
			bgColor = pDC->GetPixel( xs, (int)yt);
			...
			pDC->SetPixelV( xs, (int)yt, RGB(red,green,blue));

			bgColor = pDC->GetPixel( xs, (int)yt+1);
			...
			pDC->SetPixelV(xs, (int)yt+1, RGB(red,green,blue));
	...
	else
	{
		...
		for (ys=y1+1; ys<y2; ys++)
		{
			...
			bgColor = pDC->GetPixel( (int)xt, ys);
			red = (UCHAR)(distance*GetRValue(bgColor)) + (UCHAR)((1.0f-distance)*GetRValue(color));
			green = (UCHAR)(distance*GetGValue(bgColor)) + (UCHAR)((1.0f-distance)*GetGValue(color));
			blue = (UCHAR)(distance*GetBValue(bgColor)) + (UCHAR)((1.0f-distance)*GetBValue(color));
			pDC->SetPixelV( (int)xt, ys, RGB(red,green,blue));

			bgColor = pDC->GetPixel( (int)xt+1, ys);
			red = (UCHAR)((1.0f-distance)*GetRValue(bgColor)) + (UCHAR)(distance*GetRValue(color));
			green = (UCHAR)((1.0f-distance)*GetGValue(bgColor)) + (UCHAR)(distance*GetGValue(color));
			blue = (UCHAR)((1.0f-distance)*GetBValue(bgColor)) + (UCHAR)(distance*GetBValue(color));
			pDC->SetPixelV( (int)xt+1, ys, RGB(red,green,blue));
...
void DrawPolyline(CDC *pDC, POINT* points, int numberPoints, COLORREF color)
{
	...
	{
		DrawLine(pDC, points[i].x, points[i].y, points[i+1].x, points[i+1].y, color);
	}
}

void DrawPolygon(CDC *pDC, POINT* points, int numberPoints, COLORREF color)
{
	DrawPolyline(pDC, points, numberPoints, color);
	DrawLine(pDC, points[0].x, points[0].y, points[numberPoints-1].x, points[numberPoints-1].y, color);
}
...
void DrawEllipse(CDC *pDC, int x1, int y1, int x2, int y2, COLORREF color)
{
	...
	for (xs=-radiusX; xs<=radiusX; xs++)
	{
		...
		bgColor = pDC->GetPixel( centerX+xs, centerY+(int)yt);
		red = (UCHAR)(distance*GetRValue(bgColor)) + (UCHAR)((1.0f-distance)*GetRValue(color));
		green = (UCHAR)(distance*GetGValue(bgColor)) + (UCHAR)((1.0f-distance)*GetGValue(color));
		blue = (UCHAR)(distance*GetBValue(bgColor)) + (UCHAR)((1.0f-distance)*GetBValue(color));
		pDC->SetPixelV( centerX+xs, centerY+(int)yt, RGB(red,green,blue));

		bgColor = pDC->GetPixel( centerX+xs, centerY+(int)yt+1);
		red = (UCHAR)((1.0f-distance)*GetRValue(bgColor)) + (UCHAR)(distance*GetRValue(color));
		green = (UCHAR)((1.0f-distance)*GetGValue(bgColor)) + (UCHAR)(distance*GetGValue(color));
		blue = (UCHAR)((1.0f-distance)*GetBValue(bgColor)) + (UCHAR)(distance*GetBValue(color));
		pDC->SetPixelV( centerX+xs, centerY+(int)yt+1, RGB(red,green,blue));
	...
	for (ys=-(int)((float)radiusY/rel); ys<=(int)((float)radiusY/rel); ys++)
	{
		...
		bgColor = pDC->GetPixel( centerX+(int)xt, centerY+ys);
		red = (UCHAR)(distance*GetRValue(bgColor)) + (UCHAR)((1.0f-distance)*GetRValue(color));
		green = (UCHAR)(distance*GetGValue(bgColor)) + (UCHAR)((1.0f-distance)*GetGValue(color));
		blue = (UCHAR)(distance*GetBValue(bgColor)) + (UCHAR)((1.0f-distance)*GetBValue(color));
		pDC->SetPixelV( centerX+(int)xt, centerY+ys, RGB(red,green,blue));

		bgColor = pDC->GetPixel( centerX+(int)xt+1, centerY+ys);
		red = (UCHAR)((1.0f-distance)*GetRValue(bgColor)) + (UCHAR)(distance*GetRValue(color));
		green = (UCHAR)((1.0f-distance)*GetGValue(bgColor)) + (UCHAR)(distance*GetGValue(color));
		blue = (UCHAR)((1.0f-distance)*GetBValue(bgColor)) + (UCHAR)(distance*GetBValue(color));
		pDC->SetPixelV( centerX+(int)xt+1, centerY+ys, RGB(red,green,blue));
...
void DrawArc(CDC *pDC, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, COLORREF color)
{
	...
	//DrawEllipse( pDC, x1, y1, x2, y2, RGB( 128, 128, 128 ) );

	float endAngle		= 270 - (float)(atan2( centerX - x3, centerY - y3 ) / PI) * 180;
	float startAngle	= 270 - (float)(atan2( centerX - x4, centerY - y4 ) / PI) * 180;
...
void DrawPie(CDC *pDC, int x1, int y1, int x2, int y2, float startAngle, float endAngle, COLORREF color)
{
	...
	DrawArc(pDC, x1, y1, x2, y2, startAngle, endAngle, color);
	DrawLine(pDC, centerX, centerY, centerX+startX, centerY+startY, color);
	DrawLine(pDC, centerX, centerY, centerX+endX, centerY+endY, color);
}
...
void DrawChord(CDC *pDC, int x1, int y1, int x2, int y2, float startAngle, float endAngle, COLORREF color)
{
	...
	DrawArc(pDC, x1, y1, x2, y2, startAngle, endAngle, color);
	DrawLine(pDC, centerX+startX, centerY+startY, centerX+endX, centerY+endY, color);
}
...
void DrawRoundRect(CDC *pDC, int x1, int y1, int x2, int y2, int width, int height, COLORREF color)
{
	...
	{
		DrawEllipse(pDC, x1, y1, x2, y2, color);
	}
	else
	{
		DrawLine(pDC, x1+offsetX, y1, x2-offsetX, y1, color);
		DrawArc(pDC, x2-width, y1, x2, y1+height, 270.0f, 360.0f, color);
		DrawLine(pDC, x2, y1+offsetY, x2, y2-offsetY, color);
		DrawArc(pDC, x2-width, y2-height, x2, y2, 0.0f, 90.0f, color);
		DrawLine(pDC, x2-offsetX, y2, x1+offsetX, y2, color);
		DrawArc(pDC, x1, y2-height, x1+width, y2, 90.0f, 180.0f, color);
		DrawLine(pDC, x1, y2-offsetY, x1, y1+offsetY, color);
		DrawArc(pDC, x1, y1, x1+width, y1+height, 180.0f, 270.0f, color);
	}
}
...
void DrawBezier(CDC *pDC, POINT* points, int numPoints, COLORREF color)
{
	...
	if (numPoints >= numReqPoints)
	{
		...
		for (int i=0; i<numCurves; i++)
		{
			...
			{
				DrawLine(pDC, startPoint.x, startPoint.y, endPoint.x, endPoint.y, color);
			}
			else
			{
				distance1 = (float)sqrt(pow(controlPoint1.x-startPoint.x,2) + pow(controlPoint1.y-startPoint.y,2));
				distance2 = (float)sqrt(pow(controlPoint2.x-controlPoint1.x,2) + pow(controlPoint2.y-controlPoint1.y,2));
				distance3 = (float)sqrt(pow(endPoint.x-controlPoint2.x,2) + pow(endPoint.y-controlPoint2.y,2));
				...
				for (t=0.0f; t<=1.0f; t+=step)
				{
					...
					if (((int)xt != oldX) && ((int)yt != oldY))
					{
						...
						if (k != k1)
						{
							DrawLine(pDC, oldX, oldY, (int)xt, (int)yt, color);
				...
				if (((int)xt != oldX1) || ((int)yt != oldY1))
				{
					...
					{
						if (dy > 0)
							DrawLine(pDC, oldX, oldY, (int)xt, (int)yt+1, color);
						else
							DrawLine(pDC, oldX, oldY, (int)xt, (int)yt-1, color);
					}
					else
					{
						if (dx > 0)
							DrawLine(pDC, oldX, oldY, (int)xt+1, (int)yt, color);
						else
							DrawLine(pDC, oldX, oldY, (int)xt-1, (int)yt, color);
					}
				}
