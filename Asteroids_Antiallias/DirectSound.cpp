...
BOOL CDirectSound::CreateSoundBuffer(WAVEFORMATEX * pcmwf)
{
	...
	// if you want to control PAN and volume you need to specify other flags
	// DSBCAPS_STICKYFOCUS, DSBCAPS_CTRLPAN	, DSBCAPS_CTRLVOLUME, 	DSBCAPS_CTRLFREQUENCY , DSBCAPS_CTRLFX

	dsbdesc.dwFlags = DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME;		// assumes that the sound is played often
...
BOOL CDirectSound::CreateFromFile(LPCTSTR pszFile, CWnd *pWnd)
{
	CFile file;

	if( file.Open( pszFile, CFile::modeRead | CFile::typeBinary ) ) 
	{
		int iSize = file.GetLength();

		m_pFileContents = malloc( iSize );

		file.Read( m_pFileContents, iSize );

		file.Close();

		return Create( m_pFileContents, pWnd );
	}

	CString message;

	message.Format( "Sound file \"%s\" doesn't exist", pszFile );

	AfxMessageBox( message, 0, 0 );

	return FALSE;
}

// Volume is 0 - max, -100dB silence
// 0..-100 dB

void CDirectSound::SetVolume(int iDecibel)
{
	if( IsValid() )	m_pDsb->SetVolume( iDecibel * 100 );
}
