		...
		for(; A >= 0 && i < NUM_OF_RINGS; A -= RING_SPACING, i++ )
		{
			if( A <= 1 )
			{
				if( A < HYP_SPC_DIVISION )
				{
					...
					CPoint TL = GetPointAtPercent( (m_oCenter * Conv), (CCoord)TL, GrowthPercent );
					CPoint BR = GetPointAtPercent( (m_oCenter * Conv), (CCoord)BR, GrowthPercent );

					//DrawEllipse( pDC, TL.x, TL.y, BR.x, BR.y , GAME_COLOR );
				}
				else
				{
					double SlidePercent = ( A - HYP_SPC_DIVISION ) / ( 1 - HYP_SPC_DIVISION );
				
					CPoint TL = GetPointAtPercent( (CCoord)TL, m_oHypSpcNewPos * Conv, SlidePercent );
					CPoint BR = GetPointAtPercent( (CCoord)BR, m_oHypSpcNewPos * Conv, SlidePercent );
...
void CUfo::DrawElement( CDC *pDC, CCoord &Center, double Conv )
{
	...
	case 1:
		...
		*/
		//pDC->Ellipse( 
	case 0:
		double deg = m_oMovementVector.x * PI * 2 * 10;
